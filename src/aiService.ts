module aiService {
   ...
   */
  export function createComputerMove(board: Board, playerIndex: number, alphaBetaLimits: IAlphaBetaLimits) {
  ...
  function getNextStates(move: IMove, playerIndex: number): IMove[] {
    ...
    var result : IMove[] = gameLogic.getPossibleMoves(move[1].set.value, playerIndex);
    console.log("getNextStates = " + JSON.stringify(result));
    return result;
  }

module game{
  ...
  export function numbersTo(start: number, end: number) {
    var res : number[];
  ...
  function getColumn(row: number, col: number): number {
    var columns: number;
  ...
  function getSquareCenterXY(row: number, col: number): IPosition {
  ...
  function getIntegersTill(num: number): number[] {
    var res: number[] = [];
    for (var i = 0; i < num; i++) {
      res.push(i);
    }
  ...
  export function getPreviewSrc(): string {
